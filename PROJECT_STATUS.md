...
- Aria Wellness now includes a first-position live Shopify `Test Product` card in its `/brands/aria-wellness` storefront section. The card uses a Shopify-backed product URL, displays a temporary `Test Product` badge, and routes CTA clicks through `/r/aria-wellness/:creator_code/test-product` before forwarding to Shopify with `creator_code`, `partnerlinks_ref`, `brand_slug`, and `product_slug` query params for conversion attribution testing.
