...
- Earnings Lifecycle MVP is scaffolded before real money movement. New direct commissions, creator-network earnings, and brand-origin network earnings are stored as `pending` with a configurable 24-hour claim window, then displayed as pending, claimable, claimed, and lifetime earnings on the Creator Dashboard. Claimable creator earnings can be internally marked `claimed` when the signed-in owner has Stripe payouts enabled, creating an internal claim ledger row and claim batch id. The Creator Dashboard now includes a Payout History section sourced from `creator_earning_claims`, showing claimed amount, claimed date, batch id, status, and a Stripe transfer placeholder for future real payout movement. No Stripe transfers or live payouts are created yet.
...
- Claim Earnings MVP is an internal ledger/state transition only. `/earnings/claim` requires the signed-in creator owner and `stripe_onboarding_status = payouts_enabled`, then marks claimable direct commissions and creator-network earnings as `claimed`, records `claimed_at`, assigns a `claim_batch_id`, and inserts a `creator_earning_claims` history row. It does not create Stripe transfers or move money.
- Payout History UI is read-only and displays recent `creator_earning_claims` rows on the Creator Dashboard. Current statuses are `claimed`, with UI labels prepared for future `processing`, `paid`, and `failed` states. Stripe transfer ids are shown as a placeholder until real Stripe transfers exist.
