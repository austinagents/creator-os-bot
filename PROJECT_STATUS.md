...
- Aria Wellness now includes a first-position live Shopify `Test Product` card in its `/brands/aria-wellness` storefront section. The card uses the same product-card UI pattern as the rest of the grid, with no special badge or unique visual labeling, and its primary `Copy Link` CTA copies `/r/aria-wellness/:creator_code/test-product`; that PartnerLinks route forwards to Shopify with `creator_code`, `partnerlinks_ref`, `brand_slug`, and `product_slug` query params for conversion attribution testing.
...
- Shopify-backed product cards can store product-level destination metadata, starting with the Aria Wellness `Test Product`. Product referral forwarding preserves creator attribution from a signed-in creator or URL params such as `?creator_code=...`; if the mock brand is not yet linked to a database brand, the route still forwards to Shopify for attribution testing but cannot record a PartnerLinks click row until a matching brand exists.
