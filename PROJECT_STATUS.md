...
- Stripe Connect Express payout onboarding MVP is scaffolded in sandbox/test mode. The Creator Dashboard Total Earnings card now shows Not connected, Finish setup, Stripe connected, or Payouts enabled states based on Stripe `details_submitted`, `charges_enabled`, and `payouts_enabled`. The onboarding CTA uses a Stripe-branded indigo button with inline SVG branding and helper text: `Secure payout onboarding powered by Stripe.` This only creates/reuses a creator Stripe connected account and sends creators through hosted onboarding when setup is not already submitted; it does not move money, create transfers, calculate withdrawals, or custody creator earnings.
