...
- Homepage navigation includes a `For Creators` dropdown with `Creator Dashboard` as the first creator journey item; `/dashboard` shows a clean sign-in state when no authenticated creator session is available.
